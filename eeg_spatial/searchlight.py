...
def searchlight_accuracy(X, y, neighbor_idx, make_clf, splits, scoring="accuracy"):
    """Mean CV score per electrode neighborhood, for ONE classifier over fixed folds.
    ...
    scoring: 'accuracy' (raw, chance depends on class balance) or 'balanced' (balanced
    accuracy = mean per-class recall, chance = 1/3 regardless of imbalance).
    """
    y = np.asarray(y)
    ...
    for ch in range(n_channels):
        ...
        for train_idx, test_idx in splits:
            ...
            if scoring == "balanced":
                fold.append(balanced_accuracy_score(y[test_idx], pipe.predict(X_ch[test_idx])))
            else:
...
def run_searchlight(X, y, neighbor_idx, ch_names, classifiers=None,
                    n_folds=5, random_state=42, log_every=1, groups=None, scoring="accuracy"):
    """Searchlight CV score per electrode for each classifier.
    ...
    reproduce the original stratified-K-fold setup. `scoring` is 'accuracy' (default) or
    'balanced' (balanced accuracy, chance = 1/3 regardless of class imbalance).
    """
    ...
    for i, (name, make_clf) in enumerate(classifiers.items(), 1):
        results[name] = searchlight_accuracy(X, y, neighbor_idx, make_clf, splits, scoring=scoring)
        if log_every:
